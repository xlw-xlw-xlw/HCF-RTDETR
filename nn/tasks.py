    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in {
            ...
            if c2 != nc:  # if c2 not equal to number of classes (i.e. for Classify() output)
                c2 = make_divisible(min(c2, max_channels) * width, 3) if m is EFBlock else make_divisible(min(c2, max_channels) * width, 8)
            if m is C2fAttn:
        ...
        elif m in {P3ShallowFusion, P4MidFusion, P5DeepFusion}:
            c1 = ch[f[0]] + ch[f[1]] if isinstance(f, list) else ch[f]
            c2 = args[0] if args else c1
            args = [c1, c2] + list(args[1:]) if len(args) > 1 else [c1, c2]
        elif m in {CrackCFT, CrackCFT_P2, CrackCFT_P3, CrackCFT_P4, CrackCFT_P5}:
            # f = [vis_idx, ir_idx]，取第一个输入的通道数
            c2 = ch[f[0]] if isinstance(f, list) else ch[f]
            args = [c2]

        # Add2 模块
        elif m is Add2:
            # f = [src_idx, cft_idx]，取第一个输入的通道数
            c2 = ch[f[0]] if isinstance(f, list) else ch[f]
            args = [c2, args[1]]  # args[1] 是 index (0 或 1)
